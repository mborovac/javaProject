...
public class Mask {
	 ...
	 * a mask based on it. Mask is considered to be a non-dontCare mask.
	 * 
	 * @param mask
	 */
	...
	/**
	 * Class constructor. Constructor takes an array of valid mask values(zero, one or don't care) and creates
	 * a mask based on it. 2nd argument is a flag determining whether the mask is a don't care mask.
	 * 
	 * @param mask array of mask values, can not be null and can not contain null elements
	 * @param dontCareMask flag determining whether the mask is a don't care mask
	 */
	public Mask(MaskValue[] mask, boolean dontCareMask) {
		if(mask == null) {
		...
		this.dontCareMask = dontCareMask;
		this.used = false;
		createCoveredIndexes();
	}
	...
	public static Mask parse(String inputString) {
		...
		return new Mask(maskArray, false);
	}
	 ...
	 */
	public static Mask fromIndex(int maskSize, int index, boolean dontCareMask) {
		if(maskSize < 1) {
		...
		return new Mask(maskArray, dontCareMask);
	}
	
	/**
	 * Method creates a set of indexes that are covered by this mask.
	 */
	public void createCoveredIndexes() {
		Set<Integer> coveredIndexes = new HashSet<>();
		String maskString = this.toString();
		List<String> listOfMaskStrings = new ArrayList<>();
		listOfMaskStrings.add(maskString);
		boolean change = true;
		while(change) {
			change = false;
			List<String> stringsToRemove = new ArrayList<>();
			List<String> stringsToAdd = new ArrayList<>();
			for(int i = 0; i < listOfMaskStrings.size(); i++) {
				String currentMaskString = listOfMaskStrings.get(i);
				if(currentMaskString.contains("X")) {
					change = true;
					String temp1 = currentMaskString.replaceFirst("X", "0");
					String temp2 = currentMaskString.replaceFirst("X", "1");
					stringsToAdd.add(temp1);
					stringsToAdd.add(temp2);
					stringsToRemove.add(currentMaskString);
				}
			}
			listOfMaskStrings.removeAll(stringsToRemove);
			listOfMaskStrings.addAll(stringsToAdd);
		}
		for(String indexString: listOfMaskStrings) {
			int index = Integer.parseInt(indexString, 2);
			coveredIndexes.add(index);
		}
		this.coveredIndexes = coveredIndexes;
	}
	...
	/**
	 * Method used to acquire the value of the flag dontCareMask, it indicates whether
	 * the current mask is a don't care mask.
	 * 
	 * @return returns the value of the dontCareMask flag
	 */
	public boolean isDontCareMask() {
		return this.dontCareMask;
	}
	
	/**
	 * Method used to check whether the current mask was already used in the QMC minimization.
	 * 
	 * @return returns true if the mask was used already, false otherwise
	 */
	...
	/**
	 * Method used to set the value of the "used" flag to true.
	 */
	public void setUsed() {
		this.used = true;
	}
	...
	/**
	 * Method used to acquire the set of indexes covered by the current mask.
	 * 
	 * @return returns the set of indexes covered by the current mask
	 */
	public Set<Integer> getCoveredIndexes() {
		return this.coveredIndexes;
	}
	...
	public static Mask combine(Mask mask1, Mask mask2) {
		...
		if(differences == 0) {
			...
			if(mask1.isDontCareMask() && mask2.isDontCareMask()) {
				isDontCare = true;
			}
			Mask combinedMask = new Mask(maskArray, isDontCare);
	...
	public String toString() {
		...
		for(int i = 0; i < this.mask.length; i++) {
			if(this.mask[i] == MaskValue.ONE) {
				sb.append("1");
			} else if(this.mask[i] == MaskValue.ZERO) {
				sb.append("0");
			} else {
				sb.append("X");
			}
